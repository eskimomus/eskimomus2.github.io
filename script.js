...
const chromiumContentOffset = 100;
...
function isChromiumBrowser() {
  return document.documentElement.classList.contains("is-chromium");
}

function getChromiumContentOffset() {
  return isChromiumBrowser() ? chromiumContentOffset : 0;
}
...
function getContactMinHeight() {
  return (
    contactMinHeight +
    footerHeight +
    getChromiumContentOffset() +
    (isMobileViewport() ? contactMobileFooterOffset : 0)
  );
}
...
function syncMusicLayout() {
  ...
  const baseMusicHeight = musicMinHeight + activeFooterHeight + getChromiumContentOffset();
